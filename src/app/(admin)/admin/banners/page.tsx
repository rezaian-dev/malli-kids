...
export default function AdminBanners() {
  ...
  const list = useMemo(() => {
    ...
    return db.banners.filter((banner) => {
      const matchesSearch = !term || `${banner.occasion} ${banner.title} ${banner.subtitle} ${banner.coupon ?? ""}`.toLocaleLowerCase("fa").includes(term);
      const matchesStatus = status === "all" || (status === "active" ? banner.active : !banner.active);
      const matchesPin = pin === "all" || (pin === "pinned" ? banner.pinned : !banner.pinned);
  ...
  const activeFilters = Number(!!q.trim()) + Number(status !== "all") + Number(pin !== "all") + Number(theme !== "all");
  ...
  return (
    <div>
      ...
      <AdminStatStrip items={[
        ...
        { label: "غیرفعال", value: db.banners.length - active, Icon: EyeOff, tone: "rose" },
      ...
      <div className="rounded-[22px] max-[639px]:rounded-[19px] border border-navy/9 bg-paper/94 shadow-[0_20px_48px_-34px_rgba(14,42,71,0.38),inset_0_1px_0_rgba(255,255,255,0.72)] backdrop-blur-[18px] transition-[transform,border-color,box-shadow] duration-300 ease-[cubic-bezier(.22,1,.36,1)] hover:border-gold/40 hover:shadow-[0_24px_52px_-34px_rgba(14,42,71,0.44)] dark:border-gold-soft/16 dark:bg-[rgba(16,43,70,0.72)] dark:shadow-[0_25px_60px_-35px_rgba(0,0,0,0.76),inset_0_1px_0_rgba(255,255,255,0.045),0_0_0_1px_rgba(193,147,87,0.025)] dark:hover:border-gold-soft/30 dark:hover:shadow-[0_28px_64px_-36px_rgba(0,0,0,0.88),0_0_34px_rgba(193,147,87,0.035)] mb-5 overflow-hidden">
        <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0"><p className="text-[9px] font-black text-gold">امروز · {toFaDigits(today.jy)}/{toFaDigits(today.jm)}/{toFaDigits(today.jd)}</p><p className="mt-1 truncate text-sm font-black text-navy dark:text-ivory">{live ? `${live.occasion} — ${live.title}` : "بنر پیش‌فرض ارسال رایگان"}</p></div>
          </div>
          <span className={`w-max rounded-xl px-3 py-1.5 text-[9px] font-black ${live ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300" : "bg-navy/6 text-navy/55 dark:bg-white/6 dark:text-wheat"}`}>{live ? "در حال نمایش" : "بدون کمپین فعال"}</span>
        </div>
      </div>
      ...
      <AdminFilterBar
        ...
        onReset={() => { setQ(""); setStatus("all"); setPin("all"); setTheme("all"); }}
      >
        <AdminFilterSelect label="وضعیت نمایش" value={status} onValueChange={(value) => setStatus(value as StatusFilter)} options={[
          { value: "all", label: "همه بنرها" }, { value: "active", label: "فعال", count: active }, { value: "inactive", label: "غیرفعال", count: db.banners.length - active },
        ]} />
        <AdminFilterSelect label="اولویت" value={pin} onValueChange={(value) => setPin(value as PinFilter)} options={[
          { value: "all", label: "همه اولویت‌ها" }, { value: "pinned", label: "پین‌شده", count: pinned }, { value: "normal", label: "عادی" },
        ]} />
        <AdminFilterSelect label="تم رنگی" value={theme} onValueChange={setTheme} options={[
          { value: "all", label: "همه تم‌ها" }, { value: "navy", label: "سرمه‌ای" }, { value: "gold", label: "طلایی" }, { value: "night", label: "شب" },
        ]} />
      </AdminFilterBar>
            ...
            <article key={banner.id} className="rounded-[22px] max-[639px]:rounded-[19px] border border-navy/9 bg-paper/94 shadow-[0_20px_48px_-34px_rgba(14,42,71,0.38),inset_0_1px_0_rgba(255,255,255,0.72)] backdrop-blur-[18px] transition-[transform,border-color,box-shadow] duration-300 ease-[cubic-bezier(.22,1,.36,1)] hover:border-gold/40 hover:shadow-[0_24px_52px_-34px_rgba(14,42,71,0.44)] dark:border-gold-soft/16 dark:bg-[rgba(16,43,70,0.72)] dark:shadow-[0_25px_60px_-35px_rgba(0,0,0,0.76),inset_0_1px_0_rgba(255,255,255,0.045),0_0_0_1px_rgba(193,147,87,0.025)] dark:hover:border-gold-soft/30 dark:hover:shadow-[0_28px_64px_-36px_rgba(0,0,0,0.88),0_0_34px_rgba(193,147,87,0.035)] overflow-hidden" style={{ animationDelay: `${index * 45}ms` }}>
              ...
              <div className="p-4 sm:p-5">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                  <Field label="عنوان" value={banner.title} onChange={(value) => saveBanners(db.banners.map((item) => item.id === banner.id ? { ...item, title: value } : item))} />
                  <Field label="توضیح کوتاه" value={banner.subtitle} onChange={(value) => saveBanners(db.banners.map((item) => item.id === banner.id ? { ...item, subtitle: value } : item))} />
                  <Field label="متن دکمه" value={banner.cta} onChange={(value) => saveBanners(db.banners.map((item) => item.id === banner.id ? { ...item, cta: value } : item))} />
                  <Field label="لینک مقصد" value={banner.href} onChange={(value) => saveBanners(db.banners.map((item) => item.id === banner.id ? { ...item, href: value } : item))} />
                  <Field label="کد تخفیف" value={banner.coupon || ""} onChange={(value) => saveBanners(db.banners.map((item) => item.id === banner.id ? { ...item, coupon: value || undefined } : item))} />
                  <AdminFilterSelect label="تم بنر" value={banner.theme} onValueChange={(value) => saveBanners(db.banners.map((item) => item.id === banner.id ? { ...item, theme: value as FestiveTheme } : item))} options={[
                    { value: "navy", label: "سرمه‌ای" }, { value: "gold", label: "طلایی" }, { value: "night", label: "شب" },
...
function Field({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return <div className="min-w-0"><Label className="text-[10px] font-black text-navy/50 dark:text-wheat/75">{label}</Label><Input value={value} onChange={(event) => onChange(event.target.value)} className="mt-1.5 h-11 rounded-xl border-navy/9 bg-white/75 text-navy shadow-none dark:border-gold/16 dark:bg-navy-deep/38 dark:text-ivory" /></div>;
}
