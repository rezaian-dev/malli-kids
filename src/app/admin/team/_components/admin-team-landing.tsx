...
export function AdminTeamLanding({
  customers: initialCustomers,
}: {
  customers: AdminCustomer[];
}) {
  const [customers, , refreshCustomers] = usePolling(
    getAllCustomersAction,
    POLL_MS,
    initialCustomers,
  );
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {candidates.length === 0 ? (
              ...
              candidates.map((customer) => (
                <div
                  key={customer.id}
                  className="flex items-center justify-between gap-2 rounded-xl border px-3 py-2 border-navy/8 dark:border-gold/16"
                >
            ...
            <div className="mt-3">
              {isLastAdmin ? (
                ...
                <AdminConfirmDialog
                  ...
                  onConfirm={async () => {
                    const result = await demoteAdminAction(admin.id);
                    if (result.ok) synced();
                    return result;
                  }}
                  trigger={
