...
export const metadata = buildMetadata({
  title: "درباره ما",
  description: "قصه آتلیه ملی کیدز؛ بوتیک تخصصی پوشاک کودک با دوخت ایرانی، پارچه‌های امن و تجربه خرید دقیق‌تر.",
  path: "/about",
  ...
  return (
    <>
      ...
      <div className="container mx-auto mb-10 flex max-w-5xl flex-wrap gap-3 px-3 sm:px-5 lg:px-7">
        <Link href="/shop" className="inline-flex rounded-full bg-navy px-6 py-3 font-black text-ivory shadow-lg shadow-navy/20 transition-transform hover:-translate-y-0.5">
          مشاهده کالکشن
        </Link>
        <Link href="/tryon" className="inline-flex rounded-full border-2 border-gold px-6 py-3 font-black text-gold transition-transform hover:-translate-y-0.5">
          پرو مجازی
        </Link>
      </div>
      ...
      <div className="container mx-auto mb-14 grid max-w-5xl grid-cols-2 gap-3 px-3 sm:grid-cols-5 sm:px-5 lg:px-7">
        {ABOUT.stats.map((s) => (
          <div key={s.n} className="rounded-[26px] border border-navy/8 bg-white/94 p-4 text-center shadow-[0_18px_40px_-26px_rgba(14,42,71,.28)] transition-all duration-500 hover:-translate-y-1 hover:border-gold/50 hover:shadow-[0_22px_44px_-22px_rgba(193,147,87,.28)] dark:border-gold/30 dark:bg-slate/60">
      ...
      <section className="container mx-auto mb-14 max-w-5xl px-3 sm:px-5 lg:px-7">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {ABOUT.story.map((s) => (
            <article key={s.n} className="rounded-[26px] border border-navy/8 bg-white/94 p-5 shadow-[0_18px_40px_-26px_rgba(14,42,71,.28)] transition-all duration-500 hover:-translate-y-1 hover:border-gold/50 hover:shadow-[0_22px_44px_-22px_rgba(193,147,87,.28)] dark:border-gold/30 dark:bg-slate/60">
      ...
      <section className="container mx-auto mb-14 max-w-5xl px-3 sm:px-5 lg:px-7">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {ABOUT.promises.map((p, i) => {
            ...
            return (
              <article key={p.t} className="flex gap-4 rounded-[26px] border border-navy/8 bg-white/94 p-5 shadow-[0_18px_40px_-26px_rgba(14,42,71,.28)] transition-all duration-500 hover:-translate-y-1 hover:border-gold/50 hover:shadow-[0_22px_44px_-22px_rgba(193,147,87,.28)] dark:border-gold/30 dark:bg-slate/60">
      ...
      <section className="container relative mx-auto max-w-5xl overflow-hidden rounded-[28px] bg-navy p-8 text-cream sm:px-5 lg:px-7">
        ...
        <Link href="/contact" className="mt-5 inline-flex rounded-full bg-gold px-6 py-3 font-black text-navy-deep">
          تماس و آدرس کامل
        </Link>
      </section>
    </>
