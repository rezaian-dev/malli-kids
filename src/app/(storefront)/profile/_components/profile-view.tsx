...
const CARD =
  "mt-5 rounded-[24px] border border-navy/10 bg-white p-5 space-y-5 dark:border-gold/35 dark:bg-dusk sm:p-7";
...
export function ProfileView() {
  ...
  const account = useAppForm({
    schema: accountSchema,
    defaultValues: accountDefaults,
  });
  const child = useAppForm({
    schema: childSchema,
  ...
  useEffect(() => {
    const apply = () => {
      ...
      if (h === "orders" || h === "info" || h === "support" || h === "wishlist")
        setTab(h);
    };
    apply();
  ...
  return (
    <div className="container mx-auto w-full max-w-5xl min-w-0 px-4 pb-10 sm:px-5 lg:px-7">
      <section className="from-navy via-navy-mid to-navy-light overflow-hidden rounded-[28px] bg-linear-to-br">
        <div className="flex flex-col gap-4 px-4 py-6 sm:flex-row sm:items-center sm:px-8 sm:py-9">
          <div className="relative self-start">
            <span className="bg-navy text-gold-soft ring-gold/45 relative inline-flex size-19 items-center justify-center overflow-hidden rounded-full text-[28px] font-black ring-[3px] sm:size-24 sm:text-[34px]">
              {user.avatar ? (
                <Image
                  src={user.avatar}
                  alt=""
      ...
      >
        <TabsList className="bg-sand dark:bg-dusk-mid h-auto w-full justify-start gap-1.5 overflow-x-auto rounded-[18px] p-1.5">
          {(
            [
            ...
            <TabsTrigger
              ...
              value={id}
              className={cn(
                "text-navy dark:text-linen inline-flex h-10 items-center gap-1.5 rounded-xl px-3.5 text-[13px] font-extrabold",
        ...
        <TabsContent value="info">
          ...
          >
            ...
            <p className="text-navy/50 dark:text-wheat -mt-3 text-xs">
              اختیاری است؛ اگر پرش کنید، ما سایزِ دقیق‌تری پیشنهاد می‌دهیم.
            </p>
    ...
    <div className={CARD}>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {stats.map(({ Icon, k, v }) => (
          <div
            key={k}
            className="border-navy/10 bg-navy/2 dark:border-gold/25 rounded-2xl border px-4 py-3.5 text-center dark:bg-white/3"
          >
            <Icon className="text-gold mx-auto size-4" />
            <p
              className="text-navy dark:text-ivory mt-2 truncate text-sm font-black"
              dir="ltr"
            >
              {v}
            </p>
            <p className="text-navy/45 dark:text-wheat mt-0.5 text-[10px] font-bold">
              {k}
            </p>
          </div>
        ))}
      ...
      {orders.length === 0 ? (
        <div className="border-navy/15 dark:border-gold/25 rounded-2xl border border-dashed px-6 py-10 text-center">
          ...
          <p className="text-navy/50 dark:text-wheat mx-auto mt-1 max-w-xs text-xs leading-6">
            اولین خریدتان را ثبت کنید؛ اینجا مرحله‌به‌مرحله تا دمِ درِ خانه
            پیگیری‌اش می‌کنید.
          </p>
        ...
        <p className="bg-navy/3 text-navy/50 dark:text-wheat rounded-2xl px-4 py-6 text-center text-xs font-bold dark:bg-white/4">
...
function OrderCard({ order }: { order: Order }) {
  ...
  return (
    <li className="border-navy/10 dark:border-gold/25 overflow-hidden rounded-2xl border">
      <div className="border-navy/8 bg-navy/2 dark:border-gold/15 flex flex-wrap items-center gap-2 border-b px-4 py-3 dark:bg-white/2">
        ...
        <span className="text-navy/45 dark:text-wheat ms-auto text-[10px] font-bold">
          {order.date}
        </span>
        ...
        {order.items.map((it) => (
          <li key={`${it.id}-${it.size}`} className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-navy/50 dark:text-wheat text-[11px] font-bold">
      ...
      {stage === -1 ? (
        <p className="bg-rose/10 text-rose mx-4 mb-4 rounded-xl px-4 py-2.5 text-[11px] font-black">
                  ...
                  <span
                    className={cn(
                      "text-[9px] font-black",
                        ...
                        : "text-navy/35 dark:text-wheat/50",
                    )}
                  >
                    {label}
                  </span>
                ...
                {i < ORDER_STAGES.length - 1 ? (
                  <span
                    className={cn(
                      ...
                      i < stage ? "bg-gold" : "bg-navy/10 dark:bg-gold/20",
                    )}
                  />
                ) : null}
              </li>
...
function WishlistPanel() {
  ...
  return (
    <div className={CARD}>
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="text-navy dark:text-linen flex items-center gap-2 text-lg font-black">
            <Heart className="fill-rose text-rose size-5" /> علاقه‌مندی‌های من
          </h2>
          <p className="text-navy/50 dark:text-wheat mt-1 text-xs leading-6">
            {toFaDigits(products.length)} محصول نشانده‌اید؛ هر وقت خواستید
            سراغ‌شان برگردید.
          </p>
        </div>
      </div>
      ...
      {products.length === 0 ? (
        <div className="border-navy/15 dark:border-gold/25 rounded-2xl border border-dashed px-6 py-10 text-center">
          ...
          <p className="text-navy/50 dark:text-wheat mx-auto mt-1 max-w-xs text-xs leading-6">
            روی قلبِ هر محصول بزنید تا این‌جا برایتان نگه داشته شود.
          </p>
          <Button asChild variant="navy" className="mt-4 h-10 px-6">
    ...
    <div className={CARD}>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-navy/50 dark:text-wheat mt-1 text-xs leading-6">
            هر سوالی دارید به‌صورت تیکت بپرسید؛ پاسخِ ما فقط و فقط در پنلِ
            خودتان ثبت می‌شود.
          </p>
        </div>
        {!compose ? (
      ...
      {compose ? (
        ...
        <div className="border-navy/15 dark:border-gold/25 mt-6 rounded-2xl border border-dashed px-6 py-10 text-center">
          ...
          <p className="text-navy/50 dark:text-wheat mx-auto mt-1 max-w-xs text-xs leading-6">
            مشاورهٔ سایز، پیگیری سفارش یا هر سوالِ دیگر — تیکت بسازید تا در همین
            پنل پاسخ بگیرید.
          </p>
        </div>
      ) : (
    ...
    <div className="border-navy/8 bg-navy/2 dark:border-gold/15 space-y-3 border-t px-4 py-4 dark:bg-white/2">
      {ticket.replies.map((r, i) => (
        <div
          ...
          className={cn(
            "flex",
            r.from === "support" ? "justify-end" : "justify-start",
          )}
        >
          <div
            className={cn(
