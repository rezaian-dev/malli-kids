...
export function Collab() {
  return (
    <section className={cn(wash.cream, "cv-auto py-12 sm:py-16")}>
      <div className="container mx-auto w-full px-4 sm:px-5 lg:px-7">
        <div
          className="relative overflow-hidden rounded-3xl p-6 shadow-xl sm:rounded-[28px] sm:p-10 from-navy-deep via-navy to-navy-mid shadow-navy/20 bg-linear-to-l"
        >
          <div
            className="pointer-events-none absolute -top-24 -left-24 h-64 w-64 rounded-full blur-3xl bg-gold/15"
          />
          <div className="relative flex flex-col items-center gap-6 text-center sm:flex-row sm:justify-between sm:text-start">
            <div className="flex items-start gap-4 text-start sm:gap-5">
              <span
                className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border sm:h-16 sm:w-16 border-gold/30 bg-gold/15"
              >
              ...
              <div className="min-w-0">
                ...
                <ul className="mt-4 flex flex-wrap justify-center gap-2 sm:justify-start">
                  {WAYS.map(({ Icon, t }) => (
                    <li
                      key={t}
                      className="inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[11px] font-bold sm:text-xs text-cream/80 border-white/12 bg-white/6"
                    >
            ...
            <a
              href="/collab"
              className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full px-7 py-3.5 text-sm font-black shadow-lg transition-all duration-500 hover:-translate-y-1 sm:px-8 bg-gold text-navy-deep shadow-gold/25 hover:bg-gold-light"
            >
