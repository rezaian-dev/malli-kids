...
import { Clock, Headphones, MapPin, Phone, RotateCcw, ShieldCheck, Truck } from "lucide-react";
...
const SOCIALS = [
  {
    ...
    hover: "hover:bg-[#d62976] hover:bg-linear-to-tr hover:from-[#f9ce34] hover:via-[#ee2a7b] hover:to-[#6228d7] hover:shadow-[0_12px_28px_-10px_rgba(238,42,123,.75)]",
    Icon: InstagramIcon,
  },
  {
    name: "تلگرام",
    href: "https://t.me/mallikids",
    hover: "hover:bg-[#2aabee] hover:shadow-[0_12px_28px_-10px_rgba(42,171,238,.75)]",
    Icon: TelegramIcon,
  },
  {
    name: "واتساپ",
    href: "https://wa.me/982126401234",
    hover: "hover:bg-[#25d366] hover:shadow-[0_12px_28px_-10px_rgba(37,211,102,.75)]",
    Icon: WhatsAppIcon,
  },
  {
    name: "ایتا",
    href: "https://eitaa.com/mallikids",
    hover: "hover:bg-[#f5821f] hover:shadow-[0_12px_28px_-10px_rgba(245,130,31,.75)]",
    Icon: EitaaIcon,
  },
] as const;
      ...
      <div className={`${shell} grid grid-cols-[repeat(auto-fit,minmax(11.25rem,1fr))] gap-8 py-10 sm:py-12`}>
        <div className="min-w-0">
          <div className="flex items-center gap-3">
            <Image src="/brand/logo-white.png" alt="لوگوی ملی‌کیدز" width={56} height={56} className="size-14 object-contain" />
          ...
          <div className="mt-5 flex gap-2.5">
            {SOCIALS.map(({ name, href, hover, Icon }) => (
              <a key={name} href={href} target="_blank" rel="noreferrer" aria-label={name} className={`${SOCIAL_BASE} ${hover}`}>
        ...
        <div>
          ...
          <ul className="m-0 grid list-none gap-0.5 p-0 text-sm">
            ...
            <li><Link href="/collab" className={FOOT_LINK}>همکاری با ما</Link></li>
          </ul>
        </div>
        <div>
      ...
      <div className="border-t border-white/10">
        <div className={`${shell} flex flex-col items-center justify-between gap-6 py-8 sm:flex-row`}>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <Image src="/brand/enamad.png" alt="نماد اعتماد الکترونیکی" width={466} height={429} className="max-h-full w-auto object-contain" />
            </a>
            <a
            ...
            >
              <Image src="/brand/samandehi.png" alt="نشان ملی ثبت (ساماندهی)" width={700} height={391} className="max-h-full w-auto object-contain" />
            </a>
          </div>
        </div>
      </div>
