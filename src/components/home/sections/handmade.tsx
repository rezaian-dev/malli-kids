...
import { OrnBow } from "@/components/home/ornaments";
...
export function Handmade() {
  return (
    <section id="handmade" className="relative overflow-hidden bg-navy bg-[radial-gradient(rgba(255,255,255,.09)_1px,transparent_1px)] bg-size-[22px_22px] py-12 sm:py-16 lg:py-20">
      <div className="pointer-events-none absolute -top-32 right-1/4 h-100 w-100 rounded-full bg-gold/10 blur-3xl" />
        ...
        <div className="mb-10 flex flex-col justify-between gap-5 transition-all duration-700 ease-out sm:mb-12 sm:flex-row sm:items-end">
          <div>
            ...
            <p className="mt-3 max-w-xl text-sm leading-7 text-cream/55 sm:text-[15px]">
              هر قطعه با دستِ هنرمندان ایرانی و با کاموای درجه‌یک بافته می‌شود؛ هیچ دو قطعه‌ای دقیقاً شبیه هم نیست — درست مثل فرزند شما.
            </p>
          </div>
          <a href="/shop?cat=دستدوز" className="inline-flex w-max shrink-0 items-center gap-2 rounded-full border border-gold/40 px-5 py-2.5 text-sm font-bold text-gold-light transition-all hover:bg-gold hover:text-navy-deep sm:px-6 sm:py-3">
            مشاهده همه
            <ArrowLeft className="h-4 w-4" />
          </a>
        </div>
        <div id="handmadeGrid" className="grid grid-cols-2 gap-3 transition-all duration-700 ease-out min-[420px]:gap-4 sm:gap-6 lg:grid-cols-4">
