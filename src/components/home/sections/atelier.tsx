...
export function Atelier() {
  return (
    <section id="atelier" className="relative overflow-hidden bg-sand py-12 dark:bg-transparent sm:py-16 lg:py-20">
      <span className="font-display pointer-events-none absolute left-8 top-10 hidden -rotate-6 select-none text-[120px] leading-none text-transparent [-webkit-text-stroke:1.5px_rgba(193,147,87,.55)] xl:block">
        atelier
      </span>
      <div className="container mx-auto grid w-full items-center gap-16 px-4 sm:gap-14 sm:px-5 lg:grid-cols-2 lg:gap-10 lg:px-7">
        <div className="relative order-2 h-90 transition-all duration-700 ease-out min-[420px]:h-105 sm:h-135 lg:order-1">
          <div className="absolute right-0 top-0 w-[78%] rotate-2 overflow-hidden rounded-2xl shadow-2xl shadow-navy/20 sm:rounded-[28px]">
          ...
          <div className="absolute bottom-0 left-0 z-10 w-[52%] -rotate-3 overflow-hidden rounded-xl border-4 border-white shadow-2xl shadow-navy/25 sm:rounded-3xl sm:border-8">
          ...
          <div className="absolute left-1 top-4 z-20 h-20 w-20 animate-floaty rotate-6 overflow-hidden rounded-xl border-4 border-white shadow-xl min-[420px]:h-24 min-[420px]:w-24 sm:left-8 sm:top-6 sm:h-28 sm:w-28 sm:rounded-2xl">
            <Image
        ...
        <div className="order-1 transition-all duration-700 ease-out lg:order-2">
          ...
          <p className="mt-4 text-sm leading-7 text-navy/60 dark:text-wheat sm:mt-5 sm:text-base sm:leading-8">
            دوست دارید خودتان بدوزید؟ الگوی دقیق هر مدل را همراه با پارچه اصل همان کالکشن دریافت کنید و اثر ماندگار خودتان را بسازید.
          </p>
          <div className="mt-8 grid grid-cols-1 gap-3.5 min-[480px]:grid-cols-2 sm:mt-9 sm:gap-4">
            {CARDS.map((card) => (
              <Link
                key={card.t}
                href={card.href}
                className={`group relative rounded-2xl border border-navy/5 bg-white/92 p-5 transition-all hover:-translate-y-1 hover:border-gold/50 hover:shadow-xl hover:shadow-gold/10 dark:border-gold/25 dark:bg-slate/55 sm:rounded-3xl sm:p-6 ${card.shift ? "min-[480px]:translate-y-5" : ""}`}
              >
                <span className="absolute left-4 top-4 rounded-full bg-gold/15 px-2.5 py-1 text-[10px] font-black text-gold sm:left-5 sm:top-5">{card.tag}</span>
                <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-navy/5 transition-colors group-hover:bg-gold/15 sm:h-12 sm:w-12 sm:rounded-2xl">
                  <card.Icon className="h-5 w-5 text-navy transition-colors group-hover:text-gold dark:text-ivory sm:h-6 sm:w-6" />
                </span>
                <h3 className="mt-3.5 text-base font-black text-navy dark:text-ivory sm:mt-4 sm:text-lg">{card.t}</h3>
                <p className="mt-1.5 text-xs leading-6 text-navy/55 dark:text-wheat sm:mt-2 sm:text-sm">{card.d}</p>
              </Link>
            ))}
          </div>
          <Link href="/about" className="mt-10 inline-flex min-h-11 items-center gap-2 border-b-2 border-gold pb-1 text-sm font-black text-navy transition-colors hover:text-gold dark:text-ivory min-[480px]:mt-14 sm:text-base">
            ورود به آتلیه
