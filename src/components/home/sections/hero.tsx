...
export function Hero() {
  return (
    <section id="heroSection" className="relative overflow-hidden bg-transparent pb-14 pt-8 sm:pb-20 sm:pt-12 lg:pb-24 lg:pt-14">
      <div className="pointer-events-none absolute -inset-s-20 -top-24 size-72 animate-pulse rounded-full bg-gold/25 blur-[70px] xs:size-88" />
      <div className="pointer-events-none absolute -inset-e-24 top-32 size-80 animate-pulse rounded-full bg-navy/10 blur-[70px] xs:size-96" />
      <span className="font-display pointer-events-none absolute inset-e-[5%] top-[22%] hidden rotate-6 select-none text-[130px] leading-none text-transparent [-webkit-text-stroke:1.5px_rgba(196,163,106,.55)] xl:block">
        kids
      </span>
      <div className="relative container mx-auto grid w-full items-center gap-12 px-4 sm:px-5 lg:grid-cols-2 lg:gap-8 lg:px-7">
        <div className="relative z-10 text-center lg:text-right">
          ...
          <h1 className="animate-hero-in text-[clamp(1.75rem,7vw,4rem)] font-black leading-[1.3] text-navy delay-150 dark:text-linen">
            ...
            <span className="relative mx-1.5 inline-block text-gold sm:mx-3">
              ...
              <span className="absolute -left-2 -top-1 size-2 animate-twinkle rounded-full bg-gold shadow-[0_0_0_3px_rgba(193,147,87,0.18)]" aria-hidden />
              <OrnStar className="absolute -left-4 -top-3 h-4 w-4 sm:h-5 sm:w-5" />
            </span>
            <br />
          ...
          <p className="mx-auto mt-6 max-w-xl animate-hero-in text-[15px] leading-7 text-navy/60 delay-300 dark:text-khaki sm:mt-7 sm:text-lg sm:leading-8 lg:mx-0">
            ملی‌کیدز؛ بوتیک تخصصی پوشاک کودک با پارچه‌های ارگانیک، دوخت‌های دستی و تجربه پرو مجازی با هوش مصنوعی — برای لحظه‌هایی که قرار است در خاطره‌ها بمانند.
          </p>
          <div className="mt-8 flex animate-hero-in min-[420px]:flex-row min-[420px]:items-center flex-col flex-wrap items-stretch justify-center gap-3 delay-500 sm:mt-9 sm:gap-4 lg:justify-start">
        ...
        <div className="group/hero relative mx-auto w-full max-w-[18rem] animate-hero-in px-2 delay-200 xs:max-w-[20rem] xs:px-4 sm:max-w-110 sm:px-0">
          ...
          <div className="absolute -inset-3 -z-10 animate-orn-sway rounded-b-[40px] rounded-t-[999px] border-2 border-dashed border-gold/50 sm:-inset-5 sm:rounded-b-[48px]" />
