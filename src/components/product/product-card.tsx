...
import { AddToCart, FavButton } from "./product-card-actions";
...
export function Card({ p, view }: { p: Product; view: "grid" | "list" }) {
  ...
  const heart = <FavButton name={p.name} />;
        ...
        <div className="flex-1 min-w-0 p-3 flex flex-col gap-2">
          ...
          <div className="mt-auto flex flex-wrap items-center gap-2">
            ...
            <div className="flex gap-1.5 ms-auto">
              ...
              <AddToCart p={p} out={out} className={`${CART} h-8 px-2.5 text-[11px]`}>
                {out ? "خبرم کن" : "سبد"}
              </AddToCart>
            </div>
          </div>
  ...
  return (
    <article className="group min-w-0 overflow-hidden rounded-[24px] border border-navy/10 bg-white/94 shadow-[0_10px_28px_-18px_rgba(14,42,71,.22)] transition-all duration-500 ease-out hover:-translate-y-2 hover:border-gold/55 hover:shadow-[0_26px_48px_-18px_rgba(14,42,71,.32)] dark:border-gold-soft/35 dark:bg-slate/60 dark:shadow-none">
      <div className="relative w-full overflow-hidden bg-sand pt-[125%]">
        ...
        <div className="absolute top-2.5 left-2.5 z-3">{heart}</div>
        {p.disc ? (
          <span className="absolute top-12 left-2.5 z-2 bg-rose text-white text-[10px] font-black px-2 py-1 rounded-full whitespace-nowrap">{p.disc} تخفیف</span>
        ) : null}
        {out ? (
        ...
        <div className="absolute inset-x-2.5 bottom-2.5 z-4 hidden translate-y-[130%] flex-col gap-1.5 transition-all duration-500 ease-out group-hover:translate-y-0 min-[420px]:flex">
          ...
          <AddToCart p={p} out={out} className={`${CART} w-full text-xs py-2.5 rounded-[14px] shadow-md`}>
            {out ? (
              <>
      ...
      <div className="px-3 pt-3 pb-3.5">
        ...
        <div className="mt-2.5 grid grid-cols-2 gap-1.5 min-[420px]:hidden">
          <Link href={href} className={`${VIEW} text-[10px] py-2`}>
            <Eye width={12} height={12} /> مشاهده
          </Link>
          <AddToCart p={p} out={out} className={`${out ? "bg-rose-50 text-rose" : CART} text-[10px] py-2 rounded-[10px] font-black border-0`}>
