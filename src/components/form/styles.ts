...
export const INPUT: Record<Skin, string> = {
  ...
  inset: "h-full min-w-0 flex-1 border-0 bg-transparent px-4 shadow-none focus-visible:ring-0 dark:bg-transparent",
};
...
export const SHELL: Partial<Record<Skin, string>> = {
  ...
  inset:
    "flex h-12 items-center rounded-xl border bg-white transition-colors focus-within:border-gold focus-within:ring-2 focus-within:ring-gold/25 dark:bg-navy-deep/60 data-[invalid=true]:border-rose data-[invalid=true]:dark:border-rose border-tan dark:border-white/12",
};
